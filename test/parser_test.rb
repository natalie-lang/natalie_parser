  ...
  describe parser do
    ...
    describe '#parse' do
      ...
      it 'parses method calls with parentheses' do
        ...
        expect(parse("foo(0, a: 1, b: 'two')")).must_equal s(:call, nil, :foo, s(:lit, 0), s(:hash, s(:lit, :a), s(:lit, 1), s(:lit, :b), s(:str, 'two')))
        expect(parse('foo("a": "b")')).must_equal s(:call, nil, :foo, s(:hash, s(:lit, :a), s(:str, "b")))
        expect(parse("foo('a': 'b')")).must_equal s(:call, nil, :foo, s(:hash, s(:lit, :a), s(:str, "b")))
        expect(parse("foo('a':'b')")).must_equal s(:call, nil, :foo, s(:hash, s(:lit, :a), s(:str, "b")))
        expect(parse('foo("a#{1+1}": "b")')).must_equal s(:call, nil, :foo, s(:hash, s(:dsym, "a", s(:evstr, s(:call, s(:lit, 1), :+, s(:lit, 1)))), s(:str, "b")))
        expect(parse("foo(bar: 1 && 2)")).must_equal s(:call, nil, :foo, s(:hash, s(:lit, :bar), s(:and, s(:lit, 1), s(:lit, 2))))
        expect(parse("foo(0, 1 => 2, 3 => 4)")).must_equal s(:call, nil, :foo, s(:lit, 0), s(:hash, s(:lit, 1), s(:lit, 2), s(:lit, 3), s(:lit, 4)))
      ...
      it 'parses method calls without parentheses' do
        ...
        expect(parse('describe :enumeratorize, shared: true')).must_equal s(:call, nil, :describe, s(:lit, :enumeratorize), s(:hash, s(:lit, :shared), s(:true)))
        expect(parse("describe :enumeratorize, shared: true do\nnil\nend")).must_equal s(:iter, s(:call, nil, :describe, s(:lit, :enumeratorize), s(:hash, s(:lit, :shared), s(:true))), 0, s(:nil))
        expect(parse('foo a = b, c')).must_equal s(:call, nil, :foo, s(:lasgn, :a, s(:call, nil, :b)), s(:call, nil, :c))
